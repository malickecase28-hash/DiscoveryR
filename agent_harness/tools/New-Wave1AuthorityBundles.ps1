...
param(
    ...
    [string]$ExpectedPayloadContractSha256 = '78d5fc20758831e06d487217d0367bbaab5f25cb7eefcac8bee9446f6eea3eed',
...
if ([string]::IsNullOrWhiteSpace($PayloadContractSource)) { $PayloadContractSource = 'F:\TrinityR-research\analytical_lake\fusion_markets\xauusd\payload_manifest.json' }
...
function Select-Json([object]$Document, [string]$Pointer) {
    ...
    foreach ($part in ($Pointer.TrimStart('/') -split '/')) {
        ...
        elseif ($null -eq $current) { throw "SELECTOR_MISSING: $Pointer" }
...
function New-Authority([object]$Bundle, [string]$Target) {
    ...
    if ($null -ne $Bundle.source_file) {
        ...
        $sourceName=[IO.Path]::GetFileName($sourcePath); $sourceHash=FileSha $sourcePath; if ($Bundle.source_kind -eq 'payload_contract' -and $ExpectedPayloadContractSha256 -and $sourceHash -ne $ExpectedPayloadContractSha256.ToLowerInvariant()) { throw "SOURCE_HASH_MISMATCH: $sourceName" }; $sources.Add([ordered]@{source_id=$sourceName;source_sha256=$sourceHash}); $document=[Text.Encoding]::UTF8.GetString([IO.File]::ReadAllBytes($sourcePath))|ConvertFrom-Json
...
if ($RunSyntheticTests) {
    ...
    try {
        ...
        $PayloadContractSource=Join-Path $fixture 'payload_manifest.json'; $ExpectedPayloadContractSha256=$null; [IO.File]::WriteAllText($PayloadContractSource,$payload,[Text.UTF8Encoding]::new($false)); $testRoot=Join-Path $fixture 'out'; $spec=Get-Content -Raw $SpecPath|ConvertFrom-Json
        $null=Select-Json (@{a=$null}|ConvertTo-Json|ConvertFrom-Json) '/a'; Expect-Throw { Select-Json (@{a=1}|ConvertTo-Json|ConvertFrom-Json) '/missing' } 'SELECTOR_MISSING'; Expect-Throw { Select-Json (@{a=$null}|ConvertTo-Json|ConvertFrom-Json) '/a/b' } 'SELECTOR_MISSING'
        ...
        $ap=Get-Content -Raw (Join-Path $testRoot 'AP-001\authority.json'); $tc=Get-Content -Raw (Join-Path $testRoot 'TC-001\authority.json'); $apObject=$ap|ConvertFrom-Json; $apManifest=Get-Content -Raw (Join-Path $testRoot 'AP-001\bundle_manifest.json')|ConvertFrom-Json; $apSource=$apManifest.source_files[0]; Assert ($apSource.source_id -eq 'payload_manifest.json') 'canonical source name'; Assert ($apSource.source_sha256 -eq (FileSha $PayloadContractSource)) 'source hash'; Assert ($ap -match 'DECLARED_SERIALIZATION_CONTRACT_AVAILABLE') 'AP-001 status'; Assert ($ap -match 'drift_burst') 'AP-001 extraction'; Assert ($tc -match 'feed_health' -and $tc -match 'spread_state') 'TC-001 extraction'; Assert ($apObject.selected.PSObject.Properties.Name -contains 'drift_burst') 'AP-001 selected only'; Assert ($ap -notmatch 'emitted|market_samples|performance|payload_manifest\(') 'AP-001 leak'; Assert ($tc -notmatch 'emitted|market_samples|performance|payload_manifest\(') 'TC-001 leak'
        $ap2=Get-Content -Raw (Join-Path $testRoot 'AP-002\authority.json')|ConvertFrom-Json; Assert (@($ap2.selected.PSObject.Properties).Count -eq 0 -and @($ap2.unresolved) -contains 'UNRESOLVED_PENDING_SOURCE_APPROVAL') 'AP-002 unresolved'; $bg=Get-Content -Raw (Join-Path $testRoot 'BG-001\authority.json')|ConvertFrom-Json; Assert ($bg.selected.columns -is [array] -and @($bg.unresolved) -contains 'lineage: UNRESOLVED_PENDING_SOURCE_APPROVAL') 'BG-001 schema'
        $registry=Join-Path $fixture 'registry.json'; Copy-Item $RegistryPath $registry; Update-Registry $registry $testRoot; Verify-Registry $registry $testRoot
