...
if ($launcherText -notmatch '\.runs\\wave1-native-v3' -or $launcherText -notmatch 'TRINITYR_WORKSPACE') { throw 'Native launcher is not bound to the approved folder workflow.' }
...
$runRootPath = Join-Path $repo '.runs\wave1-native-v3'
New-Item -ItemType Directory -Path $runRootPath -Force | Out-Null
$runRoot = (Resolve-Path $runRootPath).Path
